...
function canManageLegacyInternalLayout(doc) {
    return needsLegacyInternalLayout(doc) && authStore.user?.role === 'superadmin';
}

function confirmSend(doc) {
    if (needsLegacyInternalLayout(doc)) {
        toast.add({ severity: 'warn', summary: 'ต้องให้ Superadmin กำหนดกรอบก่อนส่ง', detail: 'เอกสารภายในนี้ยังไม่มีกรอบลายเซ็นจาก Workflow', life: 4000 });
        return;
    }
    confirm.require({
            ...
            <Column field="status" header="สถานะ" sortable style="min-width: 11rem">
                <template #body="{ data }">
                    <div class="status-cell">
                        ...
                        <Tag v-if="isInternalDocument(data) && data.status === 'draft'" :value="data.layoutReady ? 'กรอบจาก Workflow' : 'ต้องให้ Superadmin กำหนดกรอบ'" :severity="data.layoutReady ? 'success' : 'warn'" />
                ...
                <template #body="{ data }">
                    <div class="action-cell">
                        ...
                        <Button
                            ...
                            :disabled="needsLegacyInternalLayout(data)"
                            v-tooltip.top="needsLegacyInternalLayout(data) ? 'กรุณาให้ Superadmin กำหนดกรอบก่อนพิมพ์' : 'พิมพ์ PDF revision ล่าสุด'"
                            :loading="isTransitioning(data.id)"
                        ...
                        <Button
                            ...
                            v-tooltip.top="'สร้างลิงก์ผู้เซ็นภายนอก'"
                        ...
                        <Button v-if="!isInternalDocument(data)" icon="pi pi-sitemap" rounded outlined severity="secondary" aria-label="ดู Flow เอกสาร" v-tooltip.top="'ตรวจสอบ Flow เอกสาร'" @click="openDocumentFlowFromRow(data)" />
                        <Button v-if="queue !== 'draft' && !isInternalDocument(data)" icon="pi pi-list" rounded outlined severity="secondary" aria-label="ตรวจสอบเอกสารอ้างอิง" v-tooltip.top="'ตรวจสอบเอกสารอ้างอิง'" @click="openReferenceCheck(data)" />
...
/* The destructive action sits apart from the rest so it is not clicked by
   reflex while scanning a row of similar circles. */
.action-destructive {
    margin-left: 0.35rem;
