...
Because the generated form can grow to multiple pages, the creator places signature boxes and the legal-notice box on the real Draft PDF, not on a fixed template. An internal draft must save that layout before it can be sent; printing the latest PDF is optional and remains recorded as an audit event. Editing creates a new immutable revision and invalidates the earlier layout. After sending, the form and layout can no longer be edited. Internal documents reuse normal signers, attachments, required attachments, saved signatures, runtime notes, history, evidence, and official-print behavior.
