...
Internal forms are fixed to one A4 page with up to 15 expense rows. The generated form has one blank approval area without pre-drawn signer cells. A superadmin configures the Workflow and drags the required signature boxes into that area once; every new internal draft snapshots those placements automatically, so the document creator never arranges signature boxes. The embedded red notice is part of the form itself and needs no legal-notice overlay. Printing the latest PDF is optional and remains an audit event. Editing creates a new immutable revision while preserving the document's signed-layout snapshot; after sending, the form can no longer be edited. Internal documents reuse normal signers, attachments, required attachments, saved signatures, runtime notes, history, evidence, and official-print behavior.
