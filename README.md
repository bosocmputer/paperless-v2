...
Internal forms are fixed to one A4 page with up to 15 expense rows. The generated approval area contains six blank cells in a 3x2 grid. PaperLess maps them to the Workflow in sequence (including every signer required by an “all signers” step), so no Signature Template, legal-notice box, or per-Draft placement is required. A Workflow needing more than six approval slots cannot be activated. Printing the latest PDF is optional and remains an audit event. Editing creates a new immutable revision with the same automatic placement; after sending, the form can no longer be edited. Internal documents reuse normal signers, attachments, required attachments, saved signatures, runtime notes, history, evidence, and official-print behavior.
