...
Root-caused in `sml-api-bybos/internal/handlers/document_image.go`: `documentImagesMaxCount = 8` and the accompanying `pageNo > 8` bound were **our own invented validation**, not a real constraint of the `sml_doc_images` table (`image_id, image_file, system_id, guid_code, image_order, image_url` - a normal one-row-per-image table with no fixed column count, confirmed by reading the actual `INSERT`). SML ERP's own UI happens to only display 8 images per document, but that's a display limitation on SML's side, not a reason for PaperLess to reject uploads - confirmed with the product owner, who was explicit this must be fixed on every shop since it's a bug affecting any document with enough attachments, not a Damrong-specific issue.

**Fix**: removed the 8-image cap and the `pageNo <= 8` bound entirely (`sml-api-bybos` PR [#2](https://github.com/bosocmputer/sml-api-bybos/pull/2), merged to `main` as `4393c84`). Kept the existing payload-size (24MB total) and per-image-size (4MB) ceilings as the real safety limits against runaway uploads. No `paperless-api`/`paperless-web` changes needed - `paperless-api`'s own `smlAttachmentMaxTotalPages = 200` safety ceiling was already far above the old fake 8-limit and needed no change.

**Verified end-to-end on Damrong** before rolling out further: called `POST /api/signing-documents/{id}/retry-sml-images` directly for the stuck document - status flipped from `completed_image_failed` to `completed`, `is_lock_record=1` confirmed via a direct `sml-api-bybos` `document-candidates` call, and directly queried `sml_doc_images` in the real `drh` SML database: **9 rows now present** for `doc_no=1EPO2609-00020` (previously 0, since every prior retry failed before any insert). No image data lost or truncated.

This is an `sml-api` only change - `paperless-api`/`paperless-web`/`db` untouched on every shop.

- **Damrong Homeplus**: deployed first, verified end-to-end with the real stuck document as described above. Release evidence `/data/paperless/releases/20260910032609-fix-remove-8image-cap-4393c84/postdeploy-checks.txt`.
- **Pui, Wirat Home Mart, Insee Construction, Amata**: deployed same-session per explicit instruction that every shop needs the fix (not Damrong-only). All confirmed via `docker inspect` image tag directly on the running container, public URL smoke HTTP 200 on each. Release evidence:
  - Pui: `/data/paperless/releases/20260910032835-fix-remove-8image-cap-4393c84/postdeploy-checks.txt`
  - Wirat Home Mart: `/data/paperless/releases/20260910032948-fix-remove-8image-cap-4393c84/postdeploy-checks.txt`
  - Insee Construction: `/data/paperless/releases/20260910033042-fix-remove-8image-cap-4393c84/postdeploy-checks.txt`
  - Amata: `/data/paperless-amata/releases/20260910033130-fix-remove-8image-cap-4393c84/postdeploy-checks.txt`

**Note for future `sml-api-bybos` work**: per instruction, all new work on this repo now branches from `origin/main` and merges back after tests pass - do not continue from the old `e949305`/`3e2ed0e` branch lineage. `origin/main` was `b2a50ed` (a merge of the department-candidates work with a separate "SML profiles" feature) at the time of this fix; this fix branched cleanly from that as `fix/remove-document-images-cap`.

## Incident - 2026-09-07 (Pui only): three prior releases never actually reached Pui despite deploy logs appearing to succeed

Customer reported no date/status/document-type filters visible at `http://45.122.49.250:8095/signing/documents/history`, despite the filters feature having been deployed and confirmed across all five shops days earlier. Investigated before assuming anything: `docker ps` on Pui showed `paperless-prod-web:c3fd1df` / `paperless-prod-api:da1d6be` still running - the exact versions from *before* the entire document-filters chain (`f7f018f`, `800f649`, `bd7375d`). `compose.yml`'s mtime was Aug 24 11:04, and no release-evidence directories existed for any of those three tags - meaning the `sed`/`pull`/`up` deploy sequence run during those sessions never actually landed on Pui, even though the terminal output at the time showed no errors and appeared to complete normally. Checked the other four shops (Damrong, Wirat, Insee, Amata) directly via `docker ps` - all four were correctly running the latest versions, so this was a Pui-specific gap, not a repo or CI problem, and not something that had also silently failed elsewhere.

The exact mechanism behind the earlier silent failure was not conclusively identified - by the time this was investigated, the relevant commands were long past and there were no error logs to inspect retroactively. What's now different going forward: every deploy step in this recovery was re-run with the underlying exit code checked explicitly (`echo $?` after `pull` and `up`, not just eyeballing the log), and the result was verified independently of the deploy log itself - `docker ps` image tags read directly from the running containers, plus live calls to the filter feature's own API (`GET /api/signing-documents/format-codes`, `GET /api/signing-documents?status=cancelled`) to confirm the *behavior* was actually present, not just the version string.

**Recovery**: re-ran the full `sed`/`pull`/`up` sequence for Pui, landing on `paperless-api:800f649` + `paperless-web:bd7375d` - the same final state as the other four shops. Verified via `docker ps` (image tags match) and live API calls (format-codes endpoint returns real data, status filter narrows results correctly). Root smoke test HTTP 200. Release evidence `/data/paperless/releases/20260907115759-recover-pui-deploy/postdeploy-checks.txt`.
...
แผนก is sourced differently per document origin, both snapshotted onto `signing_documents.department_code`/`department_name` at document-creation time (same pattern already used for `party_code`/`party_name`/`doc_date`, so historical documents predating this feature correctly show a blank department rather than requiring a backfill): for SML documents, `sml-api-bybos` now joins `ic_trans`/`ap_ar_trans.department_code` against the SML master table `erp_department_list` (`LEFT JOIN`, fails open to blank if unmatched - confirmed `erp_department_list` is empty on at least one shop's tenant, so this fail-open behavior is load-bearing, not defensive-only); for internal PaperLess documents, it's read directly from the existing ส่วนงาน/ฝ่าย/แผนก field already captured on the internal-document form (`internal_documents.go`'s `DepartmentName`, no new query/join involved). The user originally described the SML source as `c_trans.department_code`; verified directly against two shops' live SML databases that the real tables are `ic_trans`/`ap_ar_trans`, not `c_trans` - `c_trans` does not exist.

This is a three-service change: `sml-api-bybos` (new `department_code`/`department_name` fields on `document-candidates`, PR merged separately as `e949305` ahead of this release and held back for a coordinated rollout), `paperless-api` (schema migration, model fields, INSERT/SELECT/scan wiring - `ab01567`), and `paperless-web` (all three frontend items - `ab01567`).

Verified live via Playwright against Damrong before deploying further: searched `q=2RIO` on ประวัติเอกสาร, URL synced correctly, navigated away to dashboard and back to the same URL - search box correctly restored to `2RIO`. Confirmed ลำดับ/แผนก columns render in the correct position on เอกสารรอเซ็น. Confirmed the department join itself returns real data via a direct `sml-api-bybos` call for a live document (`department_code=124`, `department_name=แผนกเครื่องมือช่าง`) - existing `signing_documents` rows correctly show blank department since they predate the feature, not a bug. Confirmed rows-per-page dropdown present in the paginator. Internal-document department path verified by code inspection only (direct field assignment, no query/join) - not exercised end-to-end via a live-created internal document this session.

- **Damrong Homeplus**: deployed first, verified live via Playwright as described above. Release evidence `/data/paperless/releases/20260907060019-feat-filters-department-rowsperpage-ab01567/postdeploy-checks.txt`.
- **Pui, Wirat Home Mart, Insee Construction, Amata**: deployed same-session. All confirmed healthy via `docker ps`/`docker inspect` image-tag verification directly on the running containers (not deploy-log trust, per the Pui incident practice change above), public URL smoke HTTP 200 on each. Release evidence:
  - Pui: `/data/paperless/releases/20260907061232-feat-filters-department-rowsperpage-ab01567/postdeploy-checks.txt`
  - Wirat Home Mart: `/data/paperless/releases/20260907061350-feat-filters-department-rowsperpage-ab01567/postdeploy-checks.txt`
  - Insee Construction: `/data/paperless/releases/20260907061644-feat-filters-department-rowsperpage-ab01567/postdeploy-checks.txt`
  - Amata: `/data/paperless-amata/releases/20260907061807-feat-filters-department-rowsperpage-ab01567/postdeploy-checks.txt`
